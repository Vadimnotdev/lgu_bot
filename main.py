...
def send_welcome(message):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton(
            "Преподаватель", callback_data="search_teacher")
    )
    bot.send_message(message.chat.id, "Выберите, кого искать:",
                     reply_markup=markup)
...
def select_education_type(call):
    ...
    # Устанавливаем новый контекст поиска
    user_data[chat_id] = {"search_type": "student"}
    ...
    sent_message = bot.send_message(
        chat_id, "Выберите тип образования:", reply_markup=markup)
...
def select_week(call):
    ...
    for day in DAYS_MAPPING.keys():
        markup.add(types.InlineKeyboardButton(
            day.capitalize(), callback_data=f"day_{day}"))
    # Кнопка "Назад" на предыдущий этап
    markup.add(back_button("education_spo"))
...
def select_day(message):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton(
            "Следующая неделя", callback_data="week_next")
    )
    # Кнопка "Назад" на выбор типа образования
    markup.add(back_button("education_spo"))
...
def ask_group_name(call):
    ...
    sent_message = bot.send_message(
        call.message.chat.id, "Введите номер группы(например C7124Б. C - eng, Б - ru):", reply_markup=markup)
...
def show_schedule(call):
    ...
    try:
        ...
        bot.send_message(call.message.chat.id,
                         f"Ошибка: Файл {file_name} не найден.")
        return
    ...
    for sheet_name in workbook.sheetnames:
        ...
        if column:
            schedule = extract_schedule(
                sheet, column, day, education_type, sheet_name, group_name)
            ...
            if not schedule:
                markup = types.InlineKeyboardMarkup()
                # Кнопка "Назад" на выбор дня недели
                markup.add(back_button("back_to_day_selection"))
                bot.send_message(
                    call.message.chat.id, f"На {day.capitalize()} для группы {group_name} пар нет.", reply_markup=markup)

            else:
                markup = types.InlineKeyboardMarkup()
                # Кнопка возвращает на выбор дня недели
                markup.add(back_button("back_to_day_selection"))
                bot.send_message(call.message.chat.id, display_schedule(
                    schedule, group_name), reply_markup=markup)
    ...
    if not found_group:
        bot.send_message(
            call.message.chat.id, f"Группа '{group_name}' не найдена в файле {file_name}.")
...
def ask_teacher_name(call):
    ...
    sent_message = bot.send_message(
        call.message.chat.id, "Введите фамилию и инициалы преподавателя (например, Иванов И. И.):", reply_markup=markup)
...
def select_teacher_week_step(message):
    ...
    markup.add(
        types.InlineKeyboardButton(
            "Эта неделя", callback_data="teacher_week_this"),
        types.InlineKeyboardButton(
            "Следующая неделя", callback_data="teacher_week_next")
    )
    markup.add(back_button("search_teacher"))

    # Отправляем сообщение с выбором недели и сохраняем его ID
    sent_message = bot.send_message(
        message.chat.id, "Выберите неделю для преподавателя:", reply_markup=markup)
...
def select_teacher_week(call):
    ...
    markup.add(InlineKeyboardButton(
        "📅 Показать расписание на всю неделю", callback_data="show_week_schedule"))

    # Добавляем кнопки для дней недели (в два ряда)
    day_buttons = [InlineKeyboardButton(day.capitalize(
    ), callback_data=f"teacher_day_{day}") for day in DAYS_MAPPING.keys()]
    ...
    bot.send_message(call.message.chat.id,
                     "Выберите день недели или просмотрите расписание на всю неделю:", reply_markup=markup)
...
def show_week_schedule(call):
    ...
    if not teacher_name or not week:
        bot.send_message(
            chat_id, "Ошибка: Не выбрано имя преподавателя или неделя.")
        return
    ...
    try:
        ...
        bot.send_message(
            chat_id, f"Ошибка: файл расписания на {week} не найден.")
        return
    ...
    for sheet_name in workbook.sheetnames:
        sheet = workbook[sheet_name]
        for day in DAYS_MAPPING.keys():
            daily_schedule = search_teacher(
                sheet, sheet_name, teacher_name, target_day=day, education_type="SPO")
            schedule.extend(daily_schedule)

    if not schedule:
        bot.send_message(
            chat_id, f"На неделю для преподавателя {teacher_name} расписание отсутствует.")
        return
...
def show_teacher_schedule(call):
    ...
    file_names = ["this_spo.xlsx", "this_vo.xlsx"] if week == "this" else [
    ...
    for file_name in file_names:
        ...
        for sheet_name in workbook.sheetnames:
            ...
            results = search_teacher(
                sheet, sheet_name, teacher_name, target_day=day, education_type=education_type)
            found_data.extend(results)

    # **Добавляем сортировку по номеру пары и времени**
    found_data.sort(key=lambda x: (
        int(x.get("pair", "0")) if x.get(
            "pair", "0").isdigit() else 0,  # Сортируем по номеру пары
        # Если пары одинаковые, сортируем по началу занятия
        x.get("time", ["00:00"])[0]
    ))
    ...
    if not found_data:
        bot.send_message(
            chat_id, f"На {day.capitalize()} для преподавателя {teacher_name} пар нет.", reply_markup=markup)
    else:
        bot.send_message(chat_id, display_schedule(
            found_data, teacher_name), reply_markup=markup)
...
def go_back(call):
    ...
    if call.data == "back_to_search":
        ...
        markup.add(
            types.InlineKeyboardButton(
                "Студент", callback_data="search_student"),
            types.InlineKeyboardButton(
                "Преподаватель", callback_data="search_teacher")
        )
        bot.send_message(chat_id, "Выберите, кого искать:",
                         reply_markup=markup)
        ...
        # Кнопка "Назад" ведёт к главному меню
        markup.add(back_button("back_to_search"))
        bot.send_message(chat_id, "Выберите тип образования:",
                         reply_markup=markup)
        ...
        markup.add(back_button("back_to_search"))
        sent_message = bot.send_message(
            chat_id, "Введите фамилию и инициалы преподавателя (например, Иванов И. И.):", reply_markup=markup)
        ...
        markup = InlineKeyboardMarkup()
        for day in DAYS_MAPPING.keys():
            markup.add(InlineKeyboardButton(day.capitalize(),
                       callback_data=f"teacher_day_{day}"))
        # Назад на выбор преподавателя
        ...
        markup.add(
            types.InlineKeyboardButton(
                "Эта неделя", callback_data="week_this"),
            types.InlineKeyboardButton(
                "Следующая неделя", callback_data="week_next")
        )
        # Назад на выбор группы
        ...
        markup.add(back_button("back_to_teacher_search"))
        bot.send_message(
            chat_id, "Выберите неделю для преподавателя:", reply_markup=markup)
...
def choose_week_upload(call):
    ...
    markup.add(
        types.InlineKeyboardButton(
            "📅 Обновить текущее расописание", callback_data="update_this"),
        types.InlineKeyboardButton(
            "⏭ Расписание на следующую неделю", callback_data="update_next")
    )

    bot.send_message(chat_id, "📂 Куда загрузить расписание?",
                     reply_markup=markup)
...
def process_schedule_upload(call):
    ...
    if not schedule_type:
        bot.send_message(
            chat_id, "❌ Ошибка: Не указан тип расписания (СПО или ВО).")
        return
    ...
    if not file_id:
        bot.send_message(
            chat_id, "❌ Файл не найден. Пожалуйста, загрузите его заново.")
        return
    ...
    if call.data == "update_this":
        # Например, this_spo.xlsx или this_vo.xlsx
        file_name = f"this_{schedule_type}.xlsx"
    else:
        # Например, next_spo.xlsx или next_vo.xlsx
        file_name = f"next_{schedule_type}.xlsx"
